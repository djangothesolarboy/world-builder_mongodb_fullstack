...
router.get('/', (req, res) => {
    Character.find()
        .then(characters => res.json(characters).header("Access-Control-Allow-Origin", "https://localhost:3000"))
...
router.post('/new', verify, (req, res) => {
    ...
    newCharacter.save().then(character => res.json(character).header("Access-Control-Allow-Origin", "https://localhost:3000"))
        .catch(err => res.status(404).json(err))
});
...
router.get('/:characterId', (req, res) => {
    const character = Character.findById({ _id: req.params.characterId })
        .then(character => res.json(character).header("Access-Control-Allow-Origin", "https://localhost:3000"))
        .catch(err => res.status(404).json(err))
})
